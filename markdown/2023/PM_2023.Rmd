...
density <- round(county.data$PM10/county.data$area_km*2.59)
county.data$color <- assign.colors(density,discrete=TRUE,breaks=breaks,palette=map.colors)
N <- table(cut(density,breaks=c(0,breaks,Inf),include.lowest=TRUE))
...
text(x=curr.year,y=5500,labels="A) Primary PM2.5",pos=2,cex=1.5)
...
text(x=curr.year,y=18500,labels="B) Primary PM10",pos=2,cex=1.5)
...
text(x=curr.year,y=14800,labels="C) SO2",pos=2,cex=1.5)
...
text(x=curr.year,y=24000,labels="D) NOx",pos=2,cex=1.5)
...
legend("bottom",legend=legend.lab,pt.bg=colors[c(1,2,4)],pch=c(21,22,24),
  cex=0.75,pt.cex=1.25,bty='n',ncol=3,title="Monitoring Network (# Sites)")
```
...
abline(h=seq(5,50,5),v=c(2002:curr.year),col="white")
...
legend("topleft",legend=c("10th/90th Percentile PM10 DC","Median PM10 DC","PM10 NAAQS Level"),
...
```{r label="Figure26-species-piecharts",fig.width=10,fig.height=6,fig.align="center"}
## Calculate means and percent contributions for each site
...
## Remove sites with lower mean concentrations so that pies do not overlap
...
pie.chart(x=rep(1,6),labels=c("Sulfates","Nitrates","OC","EC","Crustal","Sea Salt"),
  radius=0.02,add=TRUE,x.ctr=0.34,y.ctr=-1.35,col=pie.colors,cex=0.75)
```
\vspace{-5truemm}
...
```{r label="Figure27-species-avgconc",fig.width=10,fig.height=7,fig.align="center"}
pm25.spec.vals <- ddply(subset(pm25.spec,year >= curr.year-2),c("site","year"),summarize,latitude=latitude[1],
...
```{r label="Figure28-species-trends",fig.width=10,fig.height=7,fig.align="center"}
layout(mat=matrix(c(1:8),4,2),width=c(1,1),height=c(1,0.1,1,0.1))
pt.col <- c("#0000FFFF","#AAAAFFFF","#FFFFFFFF","#FFAAAAFF","#FF0000FF")
pt.cex <- c(1.25,1,0.75,1,1.25); pt.pch <- c(25,25,21,24,24);
