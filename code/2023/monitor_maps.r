...
t <- ddply(co.monitors,"site",summarize,latitude=as.numeric(latitude[1]),
...
legend("bottom",legend=legend.lab,pt.bg=colors,pch=21,cex=1.5,
...
t <- ddply(no2.monitors,"site",summarize,latitude=as.numeric(latitude[1]),
  longitude=as.numeric(longitude[1]),class=min(class))
t$color <- sapply(substr(t$class,1,2),function(x) switch(x,
  NE=colors[3],NC=colors[2],SL=colors[1],colors[4]))
t <- t[order(t$class,t$site,decreasing=TRUE),]
...
t <- ddply(o3.monitors,"site",summarize,latitude=as.numeric(latitude[1]),
  longitude=as.numeric(longitude[1]),class=min(class))
t$color <- sapply(substr(t$class,1,2),function(x) switch(x,
...
t <- ddply(so2.monitors,"site",summarize,latitude=as.numeric(latitude[1]),
  longitude=as.numeric(longitude[1]),class=min(class))
t$color <- sapply(substr(t$class,1,2),function(x) switch(x,
...
pm10.monitors$class <- mapply(function(type,network) ifelse(type == "INDUSTRIAL","INDUSTRIAL",
  ifelse(network %in% c("NCORE","NATTS","PROPOSED NCORE"),"NCORE/NATTS",
  ifelse(type == "SLAMS","SLAMS","SPM/OTHER"))),
...
t <- ddply(pb.monitors,"site",summarize,latitude=as.numeric(latitude[1]),
  longitude=as.numeric(longitude[1]),type=min(monitor_type))
t$color <- sapply(tolower(t$type),switch,slams=colors[1],
  tribal=colors[2],industrial=colors[3],spm=colors[4],colors[4])
N <- table(t$type)[c(2,1,4,3)]
legend.lab <- paste(c("SLAMS","INDUSTRIAL","TRIBAL","SPM/OTHER")," (",N,")",sep="")
...
plot(x=NULL,y=NULL,type='n',axes=FALSE,xlim=c(0,1),ylim=c(0,1))
legend("bottom",legend=legend.lab,pt.bg=colors[c(1,3,2,4)],pch=21,
...
t <- ddply(pb.nonreg.monitors,c("parameter","site"),summarize,latitude=as.numeric(latitude[1]),
   longitude=as.numeric(longitude[1]),class=min(class))
...
plot(x=NULL,y=NULL,type='n',axes=FALSE,xlim=c(0,1),ylim=c(0,1))
legend("bottom",legend=legend.lab,pt.bg=colors[c(1,2,4)],pch=21,
  cex=1.5,pt.cex=2,bty='n',ncol=4,title="Monitoring Network (# Sites)")
dev.off()
