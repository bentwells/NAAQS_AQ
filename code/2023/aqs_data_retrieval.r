...
co.monitors <- get.monitors(par=42101,yr1=curr.year-2,yr2=curr.year,all=TRUE)
no2.monitors <- get.monitors(par=42602,yr1=curr.year-2,yr2=curr.year,all=TRUE)
o3.monitors <- get.monitors(par=44201,yr1=curr.year-2,yr2=curr.year,all=TRUE)
pb.monitors <- get.monitors(par=14129,yr1=curr.year-2,yr2=curr.year,all=TRUE)
pm10.monitors <- get.monitors(par=81102,yr1=curr.year-2,yr2=curr.year,all=TRUE)
pm25.monitors <- get.monitors(par=88101,yr1=curr.year-2,yr2=curr.year,all=TRUE)
so2.monitors <- get.monitors(par=42401,yr1=curr.year-2,yr2=curr.year,all=TRUE)
...
t0 <- get.monitors(par=12128,yr1=curr.year-2,yr2=curr.year,all=TRUE)[,-12]; t0$parameter <- 12128;
t1 <- get.monitors(par=82128,yr1=curr.year-2,yr2=curr.year,all=TRUE); t1$parameter <- 82128;
t2 <- get.monitors(par=85128,yr1=curr.year-2,yr2=curr.year,all=TRUE); t2$parameter <- 85128;
t3 <- get.monitors(par=88128,yr1=curr.year-2,yr2=curr.year,all=TRUE); t3$parameter <- 88128;
pb.nonreg.monitors <- rbind(t0,t1,t2,t3)
pm10.nonreg.monitors <- get.monitors(par=85101,yr1=curr.year-2,yr2=curr.year,all=TRUE)
pm25.nonreg.monitors <- get.monitors(par=88502,yr1=curr.year-2,yr2=curr.year,all=TRUE)
pm10_25.monitors <- get.monitors(par=86101,yr1=curr.year-2,yr2=curr.year,all=TRUE)
no3 <- get.monitors(par=88306,yr1=curr.year-2,yr2=curr.year,all=TRUE); no3$parameter <- 88306;
oc <- get.monitors(par=88320,yr1=curr.year-2,yr2=curr.year,all=TRUE); oc$parameter <- 88320;
ec <- get.monitors(par=88321,yr1=curr.year-2,yr2=curr.year,all=TRUE); ec$parameter <- 88321;
so4 <- get.monitors(par=88403,yr1=curr.year-2,yr2=curr.year,all=TRUE); so4$parameter <- 88403;
...
pm25.annual.mean <- get.aqs.data(paste("
  SELECT si.state_code || si.county_code || si.site_id AS site,
   ...
   WHERE ans.mo_mo_id = mo.mo_id
     ...
     AND ans.annual_summary_year <= ",curr.year,"
     AND ans.edt_edt_id IN (0,2)
     AND ans.pollutant_standard_id IN (0,27)
     AND ans.sd_duration_code IN ('7','X')
...
pm25.annual.24h <- get.aqs.data(paste("
  SELECT si.state_code || si.county_code || si.site_id AS site,
   ...
   WHERE sp.ans_ans_id = ans.ans_id
     ...
     AND ans.annual_summary_year <= ",curr.year,"
     AND ans.edt_edt_id IN (0,2)
     AND ans.pollutant_standard_id IN (0,26)
     AND ans.sd_duration_code IN ('7','X')
     AND mo.pa_parameter_code IN ('88101','88502')
     AND si.state_code != '80'
     AND sp.percentile_num = 98
...
pm25.filt.1 <- get.daily.data(par="88101",psid=26,dur="7",stat="mean",years=curr.3yr); pm25.filt.1$parameter <- "88101";
pm25.filt.2 <- get.daily.data(par="88502",psid=0,dur="7",stat="mean",years=curr.3yr); pm25.filt.2$parameter <- "88502";
pm25.cont.1 <- get.daily.data(par="88101",psid=26,dur="X",stat="mean",years=curr.3yr); pm25.cont.1$parameter <- "88101";
...
##    cat(p,y,as.character(round(Sys.time())),"\n")
##  }
##}
cat("AQS data retrievals completed.",as.character(round(Sys.time())),"\n\n")